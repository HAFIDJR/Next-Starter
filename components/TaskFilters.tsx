...
import { TASK_SEARCH_DEBOUNCE_MS } from "@/src/features/tasks/constansts";
import {
  TASK_FILTER_LABELS,
  TASK_FILTERS,
  type TaskFilter,
...
export default function TaskFilters({ filters, trashCount }: Props) {
  ...
  const navigate = useCallback(
    (next: TaskListFilters) => {
      ...
      router.replace(`${pathname}${buildTaskListQuery(next)}`, {
        scroll: false,
      });
    },
    [pathname, router],
        ...
        {TASK_FILTERS.map((filter) => {
          ...
          return (
            <button
              ...
              aria-pressed={active}
              className={`rounded-full border px-(--chip-pad-x) py-(--chip-pad-y) text-[11px] font-medium transition-all active:scale-95 ${
                active
                  ? "border-transparent bg-ink text-canvas shadow-sm"
                  : "border-line bg-surface text-muted hover:border-line-strong hover:text-ink"
              }`}
            >
