...
export default function TaskItem({
  ...
  async function submit() {
    ...
    if (!parsed.success) {
      setTitleError(
        parsed.error.issues[0]?.message ?? "Enter a valid task title.",
      );
      return;
    }
  ...
  return (
    ...
    >
      <div
        className="flex items-start gap-3 px-(--row-pad-x) py-(--row-pad-y)"
      ...
      >
        ...
        <div className="min-w-0 flex-1 space-y-1">
          {inTrash ? (
            <p className="font-medium text-muted line-through decoration-faint">
              {task.title}
            </p>
          ) : (
            <button
          ...
          {(notesPreview || due) && (
            <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px]">
              {due ? (
                <span
                  className={`inline-flex items-center gap-1 rounded-full border px-(--chip-pad-x) py-(--chip-pad-y) font-medium ${
                    task.completed
                ...
                <span className="inline-flex min-w-0 items-center gap-1 text-faint">
                  ...
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="M7 8h10M7 12h6M4.5 4.5h15v15l-3-2h-12v-13z"
                    />
                  </svg>
                ...
                >
                  ...
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="M3 12a9 9 0 1015.5-6.2M3 4v5h5"
                    />
                  </svg>
                </button>
                <button
      ...
      {isEditing ? (
        <div className="space-y-3 border-t border-line-soft px-(--row-pad-x) py-3">
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button
                ...
                onClick={submit}
                disabled={saving}
                className="inline-flex items-center gap-1.5 rounded-xl bg-ink px-3.5 py-1.5 text-xs font-medium text-canvas transition-all hover:opacity-90 active:scale-[0.98] disabled:opacity-40"
              >
