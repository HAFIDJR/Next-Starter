...
/**
 * Notes are edited here rather than in the list: the row editor already owns title
 * and due date, and Markdown needs room to breathe.
 */
export default function TaskNotesEditor({ task }: Props) {
  const router = useRouter();
  const [value, setValue] = useState(task.notes ?? "");
  // Existing notes open in preview so the page reads like a document; an empty
  // task opens straight into the textarea. Initial state only, so no effect.
          ...
          {(["edit", "preview"] as const).map((option) => (
            <button
              ...
              className={`rounded-[0.625rem] px-2.5 py-1 text-[11px] font-medium capitalize transition-all ${
                mode === option ? "bg-sunken text-ink shadow-sm" : "text-faint hover:text-muted"
              }`}
            >
              {option}
            </button>
