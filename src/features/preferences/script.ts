/**
 * Runs in <head> before the first paint, so a dark-preference user never sees a
 * flash of light, and it records the browser's real timezone for the *next*
 * request (the server cannot know it any other way).
 *
 * Kept as a string instead of a module because it must execute before any
 * bundle is parsed; `preferences-apply.ts` mirrors it for the React side.
 */
