/**
 * Timezone helpers shared by the server and the client.
 *
 * Everything in the app is stored as an absolute UTC instant (`timestamptz`).
 * "Today" is not an absolute instant, so day boundaries have to be computed in
 * the viewer's zone. These helpers do that with `Intl` instead of a date library.
 */
...
/** UTC ms for the instant that *looks like* these wall-clock parts in `timeZone`. */
export function fromZonedParts(
  ...
  // Two passes: the offset at the guess can differ from the offset at the naive
  // value when the day boundary crosses a DST transition.
  const firstGuess = naive - zoneOffsetMs(timeZone, naive);

  return naive - zoneOffsetMs(timeZone, firstGuess);
}

export function startOfZonedDay(instant: number | Date, timeZone: string): number {
...
/**
 * Calendar-correct day arithmetic: `+1 day` from a zone's last day of the month
 * lands on the 1st, and DST shifts don't slide the result by an hour.
 */
...
export function isSameZonedDay(
  ...
  return startOfZonedDay(leftDate, timeZone) === startOfZonedDay(rightDate, timeZone);
}
